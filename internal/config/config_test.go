...
func TestT06ConfigDefaultsAndBounds(t *testing.T) {
	...
	cases := map[string][]string{"PERPETUAL_MAX_REGISTRATIONS": {"0", "-1", "18446744073709551615", "nonsense"}, "PERPETUAL_WORKERS": {"0", "8", "99999999999999"}, "PERPETUAL_POOL_CONNECTIONS": {"0", "4"}, "PERPETUAL_MAX_JOBS": {"0", "-1", "1000000000", "3"}, "PERPETUAL_QUEUE_SIZE": {"0", "-1"}, "PERPETUAL_MAX_CONNECTIONS": {"0", "-1"}, "PERPETUAL_LOCK_TIMEOUT": {"0s", "-1s", "bad"}, "PERPETUAL_STATEMENT_TIMEOUT": {"0s"}, "PERPETUAL_OPERATION_TIMEOUT": {"0s"}, "PERPETUAL_SHUTDOWN_GRACE": {"0s"}, "PERPETUAL_LISTEN_ADDRESS": {"bad-address"}, "PERPETUAL_RUNTIME_DIR": {"relative/path"}}
