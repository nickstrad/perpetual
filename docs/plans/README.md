...
The [project direction](../knowledge/project-direction.md) entry records which slices are delivered.
...
The current slice has a [detailed implementation design](durable_requests/plan.md), with code sketches, TDD assignments, and a linked local `plan_state.md`.
Use the [detailed-design skill](../../.agents/skills/detailed-design/SKILL.md) to expand later slices when they become current.
...
This breakdown defines the implementation sequence.
Keep the latest design and visuals for each slice; superseded revisions remain in Git history.
The current slice design and the later slice plans define their required safety guarantees.
...
| --- | --- | --- | --- |
| [1: Durable requests](durable_requests/README.md) | Register and inspect machine intent across retries | Fixed events, durable reservations, restart, readable replay | [Diagram 1](durable_requests/architecture.md) |
...
The assigned test owner maintains testing, simulator development, and [TESTING.md](../../TESTING.md).
The design owner resolves contracts and difficult recovery decisions.
Production owners implement bounded workflows and operating-system or persistence boundaries.
A separate reviewer examines implementation, test contracts, and evidence.
Model-selection policy lives in the detailed-design skill; concrete assignments belong in each detailed plan.
...
Read [guidance](../../GUIDANCE.md), [Go style](../TIGERSTYLE.md), and [research](../TEST_RESEARCH.md) when expanding work.
