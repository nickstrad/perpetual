...
| Strict decoder I/O/bounds/object/field/closing/trailing/presence/type guards | `T01WireStrictness`, `T01WireErrorExits`: reader error, 4097-byte body, nonobject/null/empty, malformed/truncated object, every missing/null field, duplicate literal/escaped keys, unknown fields, fractions/negative/overflow and field type errors. The decoder is syntactic only; domain invalidity is rejected by `NewRequest` in the handler, covered by `T06HTTPInvalidHasNoEffects` and the handler-level `FuzzDecodeRegistration` target. |
...
| Config count parsing/range/cross-field and timeout order | Same cases independently vary each count with valid baseline; zero/negative/overflow/above-cap, positive override, workers>=pool, workers>jobs; duration parse/zero/negative, lock>=statement and statement>=operation independently. |
